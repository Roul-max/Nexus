...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "organization_invitations" ADD CONSTRAINT "organization_invitations_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "org_invite_email_idx" ON "organization_invitations" ("organization_id","email");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "invite_email_idx" ON "organization_invitations" ("email");